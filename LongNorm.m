...
if tpflag==12
    ...
    creg=['source /etc/profile;' CIVETpath '/CIVET/quarantines/Linux-x86_64/CIVET-2.1.0/progs/multispectral_stx_registration -nothreshold -clobber -single-stage -lsq6 -modeldir ' CIVETpath '/CIVET/quarantines/Linux-x86_64/share/mni-models -model mni_icbm152_t1_tal_nlin_sym_09a -source_mask targetOnly ' imt1mnc ' ' imt2mnc ' PD.mnc ' imt1xfm ' ' imt2xfm];
    system(creg);
...
elseif tpflag==13
    ...
    cregself=['source /etc/profile;mritoself -mi -lsq6 ', imt3mnc,' ', imt1mnc,' ', imt3toimt1xfm];
    system(cregself);
    cxfmcon=['source /etc/profile;xfmconcat ', imt3toimt1xfm,' ', imt1xfm,' ', imt3xfm];
    system(cxfmcon);
